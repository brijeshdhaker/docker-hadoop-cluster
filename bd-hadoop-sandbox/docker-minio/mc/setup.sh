...
setup_minio_for_wordcount() {
  echo "Creating s3 'warehouse' bucket on minio"
  mc mb s3/warehouse --ignore-existing
  ...
  for FILE_NAME in "${example_files[@]}"; do
    ...
    mc cp "$FILE_NAME".csv s3/warehouse/flights-data/
  done
  #
  echo "Uploading users.parquet to s3"
  mc cp users.parquet s3/warehouse/users-data/
