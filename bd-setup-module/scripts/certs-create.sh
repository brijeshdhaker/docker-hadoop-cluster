...
do
    ...
    keytool -keystore /apps/security/ssl/$i.keystore.jks -alias $i -certreq -file /apps/security/ssl/$i.csr -storepass confluent -keypass confluent -ext "SAN=dns:$i,dns:$i.sandbox.net,dns:localhost"
...
EOF
    ...
    keytool -export -alias $i -file /apps/security/ssl/$i.der -keystore /apps/security/ssl/$i.keystore.jks -storepass confluent
    openssl x509 -inform der -in /apps/security/ssl/$i.der -out /apps/security/ssl/$i.certificate.pem
    keytool -importkeystore -srckeystore /apps/security/ssl/$i.keystore.jks -destkeystore /apps/security/ssl/$i.keystore.p12 -deststoretype PKCS12 -deststorepass confluent -srcstorepass confluent -noprompt
    openssl pkcs12 -in /apps/security/ssl/$i.keystore.p12 -nodes -nocerts -out /apps/security/ssl/$i.key -passin pass:confluent

    cacerts_path=$(bash -c "find /usr/lib/jvm -name cacerts" | tail -1)
    keytool -noprompt -destkeystore /apps/security/ssl/$i.truststore.jks -importkeystore -srckeystore $cacerts_path -srcstorepass changeit -deststorepass confluent
